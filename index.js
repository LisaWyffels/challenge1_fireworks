...
function createScene() {
    const geo_cube = new THREE.SphereGeometry(5, 50, 50);
    ...
    loader.load('kitchen.glb', function (glb) {
        ...
        glb.scene.traverse((node) => {
            if (node.isMesh) {
                ...
                raycast_objects.push(node);
                node.userData.targetColor = new THREE.Color(0x00ff00);
            };
        });
    });
...
function paintWithSpread(object, intersectionPoint) {
    ...
    const targetColor = object.userData.targetColor ?? new THREE.Color(0xff0000);
